class QRUtil {

  constructor(QRMode, QRMaskPattern) {
    this.QRMode = QRMode;
    this.QRMaskPattern = QRMaskPattern;
    this.PATTERN_POSITION_TABLE = [
      [],
      [6, 18],
      [6, 22],
      [6, 26],
      [6, 30],
      [6, 34],
      [6, 22, 38],
      [6, 24, 42],
      [6, 26, 46],
      [6, 28, 50],
      [6, 30, 54],
      [6, 32, 58],
      [6, 34, 62],
      [6, 26, 46, 66],
      [6, 26, 48, 70],
      [6, 26, 50, 74],
      [6, 30, 54, 78],
      [6, 30, 56, 82],
      [6, 30, 58, 86],
      [6, 34, 62, 90],
      [6, 28, 50, 72, 94],
      [6, 26, 50, 74, 98],
      [6, 30, 54, 78, 102],
      [6, 28, 54, 80, 106],
      [6, 32, 58, 84, 110],
      [6, 30, 58, 86, 114],
      [6, 34, 62, 90, 118],
      [6, 26, 50, 74, 98, 122],
      [6, 30, 54, 78, 102, 126],
      [6, 26, 52, 78, 104, 130],
      [6, 30, 56, 82, 108, 134],
      [6, 34, 60, 86, 112, 138],
      [6, 30, 58, 86, 114, 142],
      [6, 34, 62, 90, 118, 146],
      [6, 30, 54, 78, 102, 126, 150],
      [6, 24, 50, 76, 102, 128, 154],
      [6, 28, 54, 80, 106, 132, 158],
      [6, 32, 58, 84, 110, 136, 162],
      [6, 26, 54, 82, 110, 138, 166],
      [6, 30, 58, 86, 114, 142, 170]
    ];
    this.G15 = (1 << 10) | (1 << 8) | (1 << 5) | (1 << 4) | (1 << 2) | (1 << 1) | (1 << 0);
    this.G18 = (1 << 12) | (1 << 11) | (1 << 10) | (1 << 9) | (1 << 8) | (1 << 5) | (1 << 2) | (1 << 0);
    this.G15_MASK = (1 << 14) | (1 << 12) | (1 << 10) | (1 << 4) | (1 << 1);
  }

  getBCHDigit(data) {
    let digit = 0;
    while (data != 0) {
      digit += 1;
      data >>>= 1;
    }
    return digit;
  }

  getBCHTypeInfo(data) {
    let d = data << 10;
    while (this.getBCHDigit(d) - this.getBCHDigit(this.G15) >= 0) {
      d ^= (this.G15 << (this.getBCHDigit(d) - this.getBCHDigit(this.G15)));
    }
    return ((data << 10) | d) ^ this.G15_MASK;
  }

  getBCHTypeNumber(data) {
    let d = data << 12;
    while (this.getBCHDigit(d) - this.getBCHDigit(this.G18) >= 0) {
      d ^= (this.G18 << (this.getBCHDigit(d) - this.getBCHDigit(this.G18)));
    }
    return (data << 12) | d;
  }

  getPatternPosition(typeNumber) {
    return this.PATTERN_POSITION_TABLE[typeNumber - 1];
  }

  getMaskFunction(maskPattern) {

    switch (maskPattern) {

      case this.QRMaskPattern.PATTERN000 :
        return function(i, j) {
          return (i + j) % 2 === 0;
        };
      case this.QRMaskPattern.PATTERN001 :
        return function(i, j) {
          return i % 2 === 0;
        };
      case this.QRMaskPattern.PATTERN010 :
        return function(i, j) {
          return j % 3 === 0;
        };
      case this.QRMaskPattern.PATTERN011 :
        return function(i, j) {
          return (i + j) % 3 === 0;
        };
      case this.QRMaskPattern.PATTERN100 :
        return function(i, j) {
          return (Math.floor(i / 2) + Math.floor(j / 3)) % 2 === 0;
        };
      case this.QRMaskPattern.PATTERN101 :
        return function(i, j) {
          return (i * j) % 2 + (i * j) % 3 === 0;
        };
      case this.QRMaskPattern.PATTERN110 :
        return function(i, j) {
          return ((i * j) % 2 + (i * j) % 3) % 2 === 0;
        };
      case this.QRMaskPattern.PATTERN111 :
        return function(i, j) {
          return ((i * j) % 3 + (i + j) % 2) % 2 === 0;
        };

      default :
        throw new Error('bad maskPattern:' + maskPattern);
    }
  }

  bits1to9(mode) {

    switch (mode) {
      case this.QRMode.MODE_NUMBER 	: return 10;
      case this.QRMode.MODE_ALPHA_NUM 	: return 9;
      case this.QRMode.MODE_8BIT_BYTE	: return 8;
      case this.QRMode.MODE_KANJI		: return 8;
      default :
        throw new Error('mode:' + mode);
    }
  }

  bits10to26(mode) {
    switch (mode) {
      case this.QRMode.MODE_NUMBER 	: return 12;
      case this.QRMode.MODE_ALPHA_NUM 	: return 11;
      case this.QRMode.MODE_8BIT_BYTE	: return 16;
      case this.QRMode.MODE_KANJI		: return 10;
      default :
        throw new Error('mode:' + mode);
    }
  }

  bits27to40(mode) {
    switch (mode) {
      case this.QRMode.MODE_NUMBER 	: return 14;
      case this.QRMode.MODE_ALPHA_NUM	: return 13;
      case this.QRMode.MODE_8BIT_BYTE	: return 16;
      case this.QRMode.MODE_KANJI		: return 12;
      default :
        throw new Error('mode:' + mode);
    }
  }

  getLengthInBits(mode, type) {
    if (type >= 1 && type < 10) {
      return this.bits1to9(mode);
    } else if (type < 27) {
      return this.bits10to26(mode);
    } else if (type < 41) {
      return this.bits27to40(mode);
    } else {
      throw new Error('type:' + type);
    }
  }

  level1a(qrcode, moduleCount, col, row) {
    let sameCount = 0;
    let dark = qrcode.isDark(row, col);
    for (let r = -1; r <= 1; r += 1) {
      if (row + r < 0 || moduleCount <= row + r) {
        continue;
      }
      for (let c = -1; c <= 1; c += 1) {

        if (col + c < 0 || moduleCount <= col + c) {
          continue;
        }

        if (r === 0 && c === 0) {
          continue;
        }

        if (dark === qrcode.isDark(row + r, col + c)) {
          sameCount += 1;
        }
      }
    }
    return sameCount;
  }

  level1(qrcode, moduleCount, lostPoint) {
    let sameCount;
    for (let row = 0; row < moduleCount; row += 1) {
      for (let col = 0; col < moduleCount; col += 1) {
        sameCount = this.level1a(qrcode, moduleCount, row, col);

        if (sameCount > 5) {
          lostPoint += (3 + sameCount - 5);
        }
      }
    }
    return lostPoint;
  }

  level2(qrcode, moduleCount, lostPoint) {
    for (let row = 0; row < moduleCount - 1; row += 1) {
      for (let col = 0; col < moduleCount - 1; col += 1) {
        let count = 0;
        if (qrcode.isDark(row, col)) {count += 1;}
        if (qrcode.isDark(row + 1, col)) {count += 1;}
        if (qrcode.isDark(row, col + 1)) {count += 1;}
        if (qrcode.isDark(row + 1, col + 1)) {count += 1;}
        if (count === 0 || count === 4) {
          lostPoint += 3;
        }
      }
    }
    return lostPoint;
  }

  level3a(qrcode, moduleCount, lostPoint) {
    for (let row = 0; row < moduleCount; row += 1) {
      for (let col = 0; col < moduleCount - 6; col += 1) {
        if (qrcode.isDark(row, col)
            && !qrcode.isDark(row, col + 1)
            &&  qrcode.isDark(row, col + 2)
            &&  qrcode.isDark(row, col + 3)
            &&  qrcode.isDark(row, col + 4)
            && !qrcode.isDark(row, col + 5)
            &&  qrcode.isDark(row, col + 6)) {
          lostPoint += 40;
        }
      }
    }
    return lostPoint;
  }

  level3b(qrcode, moduleCount, lostPoint) {
    for (let col = 0; col < moduleCount; col += 1) {
      for (let row = 0; row < moduleCount - 6; row += 1) {
        if (qrcode.isDark(row, col)
            && !qrcode.isDark(row + 1, col)
            &&  qrcode.isDark(row + 2, col)
            &&  qrcode.isDark(row + 3, col)
            &&  qrcode.isDark(row + 4, col)
            && !qrcode.isDark(row + 5, col)
            &&  qrcode.isDark(row + 6, col)) {
          lostPoint += 40;
        }
      }
    }
    return lostPoint;
  }

  level4(qrcode, moduleCount) {
    let darkCount = 0;

    for (let col = 0; col < moduleCount; col += 1) {
      for (let row = 0; row < moduleCount; row += 1) {
        if (qrcode.isDark(row, col)) {
          darkCount += 1;
        }
      }
    }
    return darkCount;
  }

  getLostPoint(qrcode) {
    let moduleCount = qrcode.getModuleCount();
    let lostPoint = 0;

    // LEVEL1
    lostPoint = this.level1(qrcode, moduleCount, lostPoint);


    // LEVEL2
    lostPoint = this.level2(qrcode, moduleCount, lostPoint);

    // LEVEL3
    lostPoint = this.level3a(qrcode, moduleCount, lostPoint);
    lostPoint = this.level3b(qrcode, moduleCount, lostPoint);

    // LEVEL4
    let darkCount = this.level4(qrcode, moduleCount);

    let ratio = Math.abs(100 * darkCount / moduleCount / moduleCount - 50) / 5;
    lostPoint += ratio * 10;

    return lostPoint;
  }

}

class QRMath {
  constructor() {
    this.EXP_TABLE = new Array(256);
    this.LOG_TABLE = new Array(256);

    // initialize tables
    for (let i = 0; i < 8; i += 1) {
      this.EXP_TABLE[i] = 1 << i;
    }
    for (let i = 8; i < 256; i += 1) {
      this.EXP_TABLE[i] = this.EXP_TABLE[i - 4]
        ^ this.EXP_TABLE[i - 5]
        ^ this.EXP_TABLE[i - 6]
        ^ this.EXP_TABLE[i - 8];
    }
    for (let i = 0; i < 255; i += 1) {
      this.LOG_TABLE[this.EXP_TABLE[i] ] = i;
    }
  }

  glog(n) {
    if (n < 1) {
      throw new Error('glog(' + n + ')');
    }
    return this.LOG_TABLE[n];
  }

  gexp(n) {
    while (n < 0) {
      n += 255;
    }
    while (n >= 256) {
      n -= 255;
    }
    return this.EXP_TABLE[n];
  }

}

class QRRSBlock {
  constructor(QRErrorCorrectLevel) {
    this.QRErrorCorrectLevel = QRErrorCorrectLevel;
    this.RS_BLOCK_TABLE = [

      // L
      // M
      // Q
      // H

      // 1
      [1, 26, 19],
      [1, 26, 16],
      [1, 26, 13],
      [1, 26, 9],

      // 2
      [1, 44, 34],
      [1, 44, 28],
      [1, 44, 22],
      [1, 44, 16],

      // 3
      [1, 70, 55],
      [1, 70, 44],
      [2, 35, 17],
      [2, 35, 13],

      // 4
      [1, 100, 80],
      [2, 50, 32],
      [2, 50, 24],
      [4, 25, 9],

      // 5
      [1, 134, 108],
      [2, 67, 43],
      [2, 33, 15, 2, 34, 16],
      [2, 33, 11, 2, 34, 12],

      // 6
      [2, 86, 68],
      [4, 43, 27],
      [4, 43, 19],
      [4, 43, 15],

      // 7
      [2, 98, 78],
      [4, 49, 31],
      [2, 32, 14, 4, 33, 15],
      [4, 39, 13, 1, 40, 14],

      // 8
      [2, 121, 97],
      [2, 60, 38, 2, 61, 39],
      [4, 40, 18, 2, 41, 19],
      [4, 40, 14, 2, 41, 15],

      // 9
      [2, 146, 116],
      [3, 58, 36, 2, 59, 37],
      [4, 36, 16, 4, 37, 17],
      [4, 36, 12, 4, 37, 13],

      // 10
      [2, 86, 68, 2, 87, 69],
      [4, 69, 43, 1, 70, 44],
      [6, 43, 19, 2, 44, 20],
      [6, 43, 15, 2, 44, 16]
    ];
  }

  qrRSBlock(totalCount, dataCount) {
    let obj = {};
    obj.totalCount = totalCount;
    obj.dataCount = dataCount;
    return obj;
  }

  getRsBlockTable(typeNumber, errorCorrectLevel) {
    switch (errorCorrectLevel) {
      case this.QRErrorCorrectLevel.L :
        return this.RS_BLOCK_TABLE[(typeNumber - 1) * 4 + 0];
      case this.QRErrorCorrectLevel.M :
        return this.RS_BLOCK_TABLE[(typeNumber - 1) * 4 + 1];
      case this.QRErrorCorrectLevel.Q :
        return this.RS_BLOCK_TABLE[(typeNumber - 1) * 4 + 2];
      case this.QRErrorCorrectLevel.H :
        return this.RS_BLOCK_TABLE[(typeNumber - 1) * 4 + 3];
      default :
        return undefined;
    }
  }

  getRSBlocks(typeNumber, errorCorrectLevel) {
    let rsBlock = this.getRsBlockTable(typeNumber, errorCorrectLevel);

    if (typeof rsBlock === 'undefined') {
      throw new Error('bad rs block @ typeNumber:' + typeNumber +
          '/errorCorrectLevel:' + errorCorrectLevel);
    }

    let length = rsBlock.length / 3;

    let list = new Array();

    for (let i = 0; i < length; i += 1) {

      let count = rsBlock[i * 3 + 0];
      let totalCount = rsBlock[i * 3 + 1];
      let dataCount = rsBlock[i * 3 + 2];

      for (let j = 0; j < count; j += 1) {
        list.push(this.qrRSBlock(totalCount, dataCount));
      }
    }

    return list;
  }
}


/**
 * QRCodeGenerator
 * @param typeNumber 1 to 10
 * @param errorCorrectLevel 'L','M','Q','H'
 */
class QRCodeGenerator  {
  constructor(typeNumber, errorCorrectLevel) {
    this.QRMode = {
      MODE_NUMBER:		1 << 0,
      MODE_ALPHA_NUM:	1 << 1,
      MODE_8BIT_BYTE:	1 << 2,
      MODE_KANJI:		  1 << 3
    };
    this.QRErrorCorrectLevel = {
      L: 1,
      M: 0,
      Q: 3,
      H: 2
    };
    this.QRMaskPattern = {
      PATTERN000: 0,
      PATTERN001: 1,
      PATTERN010: 2,
      PATTERN011: 3,
      PATTERN100: 4,
      PATTERN101: 5,
      PATTERN110: 6,
      PATTERN111: 7
    };
    this.PAD0 = 0xEC;
    this.PAD1 = 0x11;
    this._typeNumber = typeNumber;
    this._errorCorrectLevel = this.QRErrorCorrectLevel[errorCorrectLevel];
    this._modules = null;
    this._moduleCount = 0;
    this._dataCache = null;
    this._dataList = new Array();

    this.objQRSBlock = new QRRSBlock(this.QRErrorCorrectLevel);
    this.objQRMath = new QRMath();
    this.objQRUtil = new QRUtil(this.QRMode, this.QRMaskPattern);
  }

  qrPolynomial(num, shift) {

    if (typeof num.length === 'undefined') {
      throw new Error(num.length + '/' + shift);
    }

    let _num = (function() {
      let offset = 0;
      while (offset < num.length && num[offset] === 0) {
        offset += 1;
      }
      let _num = new Array(num.length - offset + shift);
      for (let i = 0; i < num.length - offset; i += 1) {
        _num[i] = num[i + offset];
      }
      return _num;
    }());

    let obj = {};

    obj.getAt = function(index) {
      return _num[index];
    };

    obj.getLength = () => {
      return _num.length;
    };

    obj.multiply = (e) => {
      num = new Array(obj.getLength() + e.getLength() - 1);

      for (let i = 0; i < obj.getLength(); i += 1) {
        for (let j = 0; j < e.getLength(); j += 1) {
          num[i + j] ^= this.objQRMath.gexp(this.objQRMath.glog(obj.getAt(i)) + this.objQRMath.glog(e.getAt(j)));
        }
      }
      return this.qrPolynomial(num, 0);
    };

    obj.mod = (e) => {
      if (obj.getLength() - e.getLength() < 0) {
        return obj;
      }

      let ratio = this.objQRMath.glog(obj.getAt(0)) - this.objQRMath.glog(e.getAt(0));

      num = new Array(obj.getLength());
      for (let i = 0; i < obj.getLength(); i += 1) {
        num[i] = obj.getAt(i);
      }

      for (let i = 0; i < e.getLength(); i += 1) {
        num[i] ^= this.objQRMath.gexp(this.objQRMath.glog(e.getAt(i)) + ratio);
      }

      // recursive call
      return this.qrPolynomial(num, 0).mod(e);
    };

    return obj;
  }


  getModules(moduleCount) {
    let modules = new Array(moduleCount);
    for (let row = 0; row < moduleCount; row += 1) {
      modules[row] = new Array(moduleCount);
      for (let col = 0; col < moduleCount; col += 1) {
        modules[row][col] = null;
      }
    }
    return modules;
  }

  makeImpl(test, maskPattern) {
    this._moduleCount = this._typeNumber * 4 + 17;
    this._modules = this.getModules(this._moduleCount);

    this.setupPositionProbePattern(0, 0);
    this.setupPositionProbePattern(this._moduleCount - 7, 0);
    this.setupPositionProbePattern(0, this._moduleCount - 7);
    this.setupPositionAdjustPattern();
    this.setupTimingPattern();
    this.setupTypeInfo(test, maskPattern);

    if (this._typeNumber >= 7) {
      this.setupTypeNumber(test);
    }

    if (this._dataCache === null) {
      this._dataCache = this.createData(this._typeNumber, this._errorCorrectLevel, this._dataList);
    }

    this.mapData(this._dataCache, maskPattern);
  }

  conditionRC1(r, c) {
    return (r >= 0 && r <= 6 && (c === 0 || c === 6));
  }

  conditionRC2(r, c) {
    return (c >= 0 && c <= 6 && (r === 0 || r === 6));
  }

  conditionRC3(r, c) {
    return (r >= 2 && r <= 4 && c >= 2 && c <= 4);
  }

  calcByRAndC(row, col, r, c) {
    if (this.conditionRC1(r, c) || this.conditionRC2(r, c) || this.conditionRC3(r, c)) {
      this._modules[row + r][col + c] = true;
    } else {
      this._modules[row + r][col + c] = false;
    }
  }

  setupPositionProbePattern(row, col) {

    for (let r = -1; r <= 7; r += 1) {

      if (row + r <= -1 || this._moduleCount <= row + r) {continue;}

      for (let c = -1; c <= 7; c += 1) {

        if (col + c <= -1 || this._moduleCount <= col + c) {continue;}

        this.calcByRAndC(row, col, r, c);
      }
    }
  }

  getBestMaskPattern() {
    let minLostPoint = 0;
    let pattern = 0;

    for (let i = 0; i < 8; i += 1) {

      this.makeImpl(true, i);

      let lostPoint = this.objQRUtil.getLostPoint(this);

      if (i === 0 || minLostPoint > lostPoint) {
        minLostPoint = lostPoint;
        pattern = i;
      }
    }

    return pattern;
  }

  setupTimingPattern() {
    for (let r = 8; r < this._moduleCount - 8; r += 1) {
      if (this._modules[r][6] !== null) {
        continue;
      }
      this._modules[r][6] = (r % 2 === 0);
    }

    for (let c = 8; c < this._moduleCount - 8; c += 1) {
      if (this._modules[6][c] !== null) {
        continue;
      }
      this._modules[6][c] = (c % 2 === 0);
    }
  }

  calcR(row, col) {
    for (let r = -2; r <= 2; r += 1) {
      for (let c = -2; c <= 2; c += 1) {
        if (r === -2 || r === 2 || c === -2 || c === 2
            || (r === 0 && c === 0)) {
          this._modules[row + r][col + c] = true;
        } else {
          this._modules[row + r][col + c] = false;
        }
      }
    }
  }

  setupPositionAdjustPattern() {
    let pos = this.objQRUtil.getPatternPosition(this._typeNumber);

    for (let i = 0; i < pos.length; i += 1) {
      for (let j = 0; j < pos.length; j += 1) {
        let row = pos[i];
        let col = pos[j];
        if (this._modules[row][col] !== null) {
          continue;
        }
        this.calcR(row, col);
      }
    }
  }

  setupTypeNumber(test) {
    let bits = this.objQRUtil.getBCHTypeNumber(this._typeNumber);
    let i;
    let mod;
    for (i = 0; i < 18; i += 1) {
      mod = (!test && ((bits >> i) & 1) == 1);
      this._modules[Math.floor(i / 3)][i % 3 + this._moduleCount - 8 - 3] = mod;
    }

    for (i = 0; i < 18; i += 1) {
      mod = (!test && ((bits >> i) & 1) == 1);
      this._modules[i % 3 + this._moduleCount - 8 - 3][Math.floor(i / 3)] = mod;
    }
  }

  setupTypeInfo(test, maskPattern) {

    let data = (this._errorCorrectLevel << 3) | maskPattern;
    let bits = this.objQRUtil.getBCHTypeInfo(data);

    let i;
    let mod;

    // vertical
    for (i = 0; i < 15; i += 1) {

      mod = (!test && ((bits >> i) & 1) == 1);

      if (i < 6) {
        this._modules[i][8] = mod;
      } else if (i < 8) {
        this._modules[i + 1][8] = mod;
      } else {
        this._modules[this._moduleCount - 15 + i][8] = mod;
      }
    }

    // horizontal
    for (i = 0; i < 15; i += 1) {

      mod = (!test && ((bits >> i) & 1) == 1);

      if (i < 8) {
        this._modules[8][this._moduleCount - i - 1] = mod;
      } else if (i < 9) {
        this._modules[8][15 - i - 1 + 1] = mod;
      } else {
        this._modules[8][15 - i - 1] = mod;
      }
    }

    // fixed module
    this._modules[this._moduleCount - 8][8] = (!test);
  }

  mapData(data, maskPattern) {

    let inc = -1;
    let row = this._moduleCount - 1;
    let bitIndex = 7;
    let byteIndex = 0;
    let maskFunc = this.objQRUtil.getMaskFunction(maskPattern);

    for (let col = this._moduleCount - 1; col > 0; col -= 2) {

      if (col === 6) {col -= 1;}

      let condition = true;
      while (condition) {

        for (let c = 0; c < 2; c += 1) {

          if (this._modules[row][col - c] === null) {

            let dark = false;

            if (byteIndex < data.length) {
              dark = (((data[byteIndex] >>> bitIndex) & 1) == 1);
            }

            let mask = maskFunc(row, col - c);

            if (mask) {
              dark = !dark;
            }

            this._modules[row][col - c] = dark;
            bitIndex -= 1;

            if (bitIndex === -1) {
              byteIndex += 1;
              bitIndex = 7;
            }
          }
        }

        row += inc;

        if (row < 0 || this._moduleCount <= row) {
          row -= inc;
          inc = -inc;
          break;
        }
      }
    }
  }

  getErrorCorrectPolynomial(errorCorrectLength) {
    let a = this.qrPolynomial([ 1 ], 0);
    for (let i = 0; i < errorCorrectLength; i += 1) {
      a = a.multiply(this.qrPolynomial([1, this.objQRMath.gexp(i)], 0));
    }
    return a;
  }

  getData(rsBlocks, buffer) {
    let offset = 0;
    let dcdata = new Array(rsBlocks.length);
    let ecdata = new Array(rsBlocks.length);
    let maxDcCount = 0;
    let maxEcCount = 0;
    for (let r = 0; r < rsBlocks.length; r += 1) {

      let dcCount = rsBlocks[r].dataCount;
      let ecCount = rsBlocks[r].totalCount - dcCount;

      maxDcCount = Math.max(maxDcCount, dcCount);
      maxEcCount = Math.max(maxEcCount, ecCount);

      dcdata[r] = new Array(dcCount);

      for (let i = 0; i < dcdata[r].length; i += 1) {
        dcdata[r][i] = 0xff & buffer.getBuffer()[i + offset];
      }
      offset += dcCount;

      let rsPoly = this.getErrorCorrectPolynomial(ecCount);
      let rawPoly = this.qrPolynomial(dcdata[r], rsPoly.getLength() - 1);

      let modPoly = rawPoly.mod(rsPoly);
      ecdata[r] = new Array(rsPoly.getLength() - 1);
      for (let i = 0; i < ecdata[r].length; i += 1) {
        let modIndex = i + modPoly.getLength() - ecdata[r].length;
        ecdata[r][i] = (modIndex >= 0) ? modPoly.getAt(modIndex) : 0;
      }
    }

    return {ecdata: ecdata, dcdata: dcdata, maxDcCount: maxDcCount, maxEcCount: maxEcCount};
  }

  createBytes(buffer, rsBlocks) {
    let {ecdata, dcdata, maxDcCount, maxEcCount} = this.getData(rsBlocks, buffer);

    let totalCodeCount = 0;
    for (let i = 0; i < rsBlocks.length; i += 1) {
      totalCodeCount += rsBlocks[i].totalCount;
    }

    let data = new Array(totalCodeCount);
    let index = 0;

    for (let i = 0; i < maxDcCount; i += 1) {
      for (let r = 0; r < rsBlocks.length; r += 1) {
        if (i < dcdata[r].length) {
          data[index] = dcdata[r][i];
          index += 1;
        }
      }
    }

    for (let i = 0; i < maxEcCount; i += 1) {
      for (let r = 0; r < rsBlocks.length; r += 1) {
        if (i < ecdata[r].length) {
          data[index] = ecdata[r][i];
          index += 1;
        }
      }
    }

    return data;
  }

  qrBitBuffer() {
    let _buffer = new Array();
    let _length = 0;
    let obj = {};

    obj.getBuffer = () => {
      return _buffer;
    };

    obj.getAt = function(index) {
      let bufIndex = Math.floor(index / 8);
      return ((_buffer[bufIndex] >>> (7 - index % 8)) & 1) == 1;
    };

    obj.put = function(num, length) {
      for (let i = 0; i < length; i += 1) {
        obj.putBit(((num >>> (length - i - 1)) & 1) == 1);
      }
    };

    obj.getLengthInBits = () => {
      return _length;
    };

    obj.putBit = function(bit) {
      let bufIndex = Math.floor(_length / 8);
      if (_buffer.length <= bufIndex) {
        _buffer.push(0);
      }
      if (bit) {
        _buffer[bufIndex] |= (0x80 >>> (_length % 8));
      }
      _length += 1;
    };

    return obj;
  }

  createData(typeNumber, errorCorrectLevel, dataList) {

    let rsBlocks = this.objQRSBlock.getRSBlocks(typeNumber, errorCorrectLevel);

    let buffer = this.qrBitBuffer();

    let i;
    for (i = 0; i < dataList.length; i += 1) {
      let data = dataList[i];
      buffer.put(data.getMode(), 4);
      buffer.put(data.getLength(), this.objQRUtil.getLengthInBits(data.getMode(), typeNumber));
      data.write(buffer);
    }

    // calc num max data.
    let totalDataCount = 0;
    for (i = 0; i < rsBlocks.length; i += 1) {
      totalDataCount += rsBlocks[i].dataCount;
    }

    if (buffer.getLengthInBits() > totalDataCount * 8) {
      throw new Error('code length overflow. ('
        + buffer.getLengthInBits()
        + '>'
        + totalDataCount * 8
        + ')');
    }

    // end code
    if (buffer.getLengthInBits() + 4 <= totalDataCount * 8) {
      buffer.put(0, 4);
    }

    // padding
    while (buffer.getLengthInBits() % 8 !== 0) {
      buffer.putBit(false);
    }

    // padding
    let condition = true;
    while (condition) {

      if (buffer.getLengthInBits() >= totalDataCount * 8) {
        break;
      }
      buffer.put(this.PAD0, 8);

      if (buffer.getLengthInBits() >= totalDataCount * 8) {
        break;
      }
      buffer.put(this.PAD1, 8);
    }

    return this.createBytes(buffer, rsBlocks);
  }

  addData(data) {
    let newData = this.qr8BitByte(data);
    this._dataList.push(newData);
    this._dataCache = null;
  }

  isDark(row, col) {
    if (row < 0 || this._moduleCount <= row || col < 0 || this._moduleCount <= col) {
      throw new Error(row + ',' + col);
    }
    return this._modules[row][col];
  }

  getModuleCount() {
    return this._moduleCount;
  }

  make() {
    this.makeImpl(false, this.getBestMaskPattern());
  }

  createTableTag(cellSize, margin) {
    cellSize = cellSize || 2;
    margin = (typeof margin === 'undefined') ? cellSize * 4 : margin;

    let qrHtml = '';

    qrHtml += '<table style="';
    qrHtml += ' border-width: 0px; border-style: none;';
    qrHtml += ' border-collapse: collapse;';
    qrHtml += ' padding: 0px; margin: ' + margin + 'px;';
    qrHtml += '">';
    qrHtml += '<tbody>';

    for (let r = 0; r < this.getModuleCount(); r += 1) {

      qrHtml += '<tr>';

      for (let c = 0; c < this.getModuleCount(); c += 1) {
        qrHtml += '<td style="';
        qrHtml += ' border-width: 0px; border-style: none;';
        qrHtml += ' border-collapse: collapse;';
        qrHtml += ' padding: 0px; margin: 0px;';
        qrHtml += ' width: ' + cellSize + 'px;';
        qrHtml += ' height: ' + cellSize + 'px;';
        qrHtml += ' background-color: ';
        qrHtml += this.isDark(r, c) ? '#000000' : '#ffffff';
        qrHtml += ';';
        qrHtml += '"/>';
      }

      qrHtml += '</tr>';
    }

    qrHtml += '</tbody>';
    qrHtml += '</table>';

    return qrHtml;
  }

  byteArrayOutputStream() {
    let _bytes = new Array();
    let obj = {};

    obj.writeByte = (b) => {
      _bytes.push(b & 0xff);
    };

    obj.writeShort = (i) => {
      obj.writeByte(i);
      obj.writeByte(i >>> 8);
    };

    obj.writeBytes = (b, off, len) => {
      off = off || 0;
      len = len || b.length;
      for (let i = 0; i < len; i += 1) {
        obj.writeByte(b[i + off]);
      }
    };

    obj.writeString = (s) => {
      for (let i = 0; i < s.length; i += 1) {
        obj.writeByte(s.charCodeAt(i));
      }
    };

    obj.toByteArray = () => {
      return _bytes;
    };

    obj.toString = () => {
      let s = '';
      s += '[';
      for (let i = 0; i < _bytes.length; i += 1) {
        if (i > 0) {
          s += ',';
        }
        s += _bytes[i];
      }
      s += ']';
      return s;
    };

    return obj;
  }

  base64EncodeOutputStream() {
    let _buffer = 0;
    let _buflen = 0;
    let _length = 0;
    let _base64 = '';

    let obj = {};

    let encode = (n) => {
      if (n < 0) {
        // error.
      } else if (n < 26) {
        return 0x41 + n;
      } else if (n < 52) {
        return 0x61 + (n - 26);
      } else if (n < 62) {
        return 0x30 + (n - 52);
      } else if (n === 62) {
        return 0x2b;
      } else if (n === 63) {
        return 0x2f;
      }
      throw new Error('n:' + n);
    };

    let writeEncoded = (b) => {
      _base64 += String.fromCharCode(encode(b & 0x3f));
    };

    obj.writeByte = (n) => {
      _buffer = (_buffer << 8) | (n & 0xff);
      _buflen += 8;
      _length += 1;

      while (_buflen >= 6) {
        writeEncoded(_buffer >>> (_buflen - 6));
        _buflen -= 6;
      }
    };

    obj.flush = () => {
      if (_buflen > 0) {
        writeEncoded(_buffer << (6 - _buflen));
        _buffer = 0;
        _buflen = 0;
      }

      if (_length % 3 !== 0) {
        // padding
        let padlen = 3 - _length % 3;
        for (let i = 0; i < padlen; i += 1) {
          _base64 += '=';
        }
      }
    };

    obj.toString = () => {
      return _base64;
    };

    return obj;
  }

  gifImage(width, height) {
    let _width = width;
    let _height = height;
    let _data = new Array(width * height);

    let obj = {};

    let lzwTable = () => {
      let _map = {};
      let _size = 0;

      let obj2 = {};

      obj2.add = (key) => {
        if (obj2.contains(key)) {
          throw new Error('dup key:' + key);
        }
        _map[key] = _size;
        _size += 1;
      };

      obj2.size = () => {
        return _size;
      };

      obj2.indexOf = (key) => {
        return _map[key];
      };

      obj2.contains = (key) => {
        return typeof _map[key] !== 'undefined';
      };

      return obj2;
    };

    let bitOutputStream = (out) => {
      let _out = out;
      let _bitLength = 0;
      let _bitBuffer = 0;

      let obj3 = {};

      obj3.write = (data, length) => {

        if ((data >>> length) != 0) {
          throw new Error('length over');
        }

        while (_bitLength + length >= 8) {
          _out.writeByte(0xff & ((data << _bitLength) | _bitBuffer));
          length -= (8 - _bitLength);
          data >>>= (8 - _bitLength);
          _bitBuffer = 0;
          _bitLength = 0;
        }

        _bitBuffer = (data << _bitLength) | _bitBuffer;
        _bitLength = _bitLength + length;
      };

      obj3.flush = () => {
        if (_bitLength > 0) {
          _out.writeByte(_bitBuffer);
        }
      };

      return obj3;
    };

    obj.setPixel = (x, y, pixel) => {
      _data[y * _width + x] = pixel;
    };

    obj.write = (out) => {

      //---------------------------------
      // GIF Signature

      out.writeString('GIF87a');

      //---------------------------------
      // Screen Descriptor

      out.writeShort(_width);
      out.writeShort(_height);

      out.writeByte(0x80); // 2bit
      out.writeByte(0);
      out.writeByte(0);

      //---------------------------------
      // Global Color Map

      // black
      out.writeByte(0x00);
      out.writeByte(0x00);
      out.writeByte(0x00);

      // white
      out.writeByte(0xff);
      out.writeByte(0xff);
      out.writeByte(0xff);

      //---------------------------------
      // Image Descriptor

      out.writeString(',');
      out.writeShort(0);
      out.writeShort(0);
      out.writeShort(_width);
      out.writeShort(_height);
      out.writeByte(0);

      //---------------------------------
      // Local Color Map

      //---------------------------------
      // Raster Data

      let lzwMinCodeSize = 2;
      let raster = getLZWRaster(lzwMinCodeSize);

      out.writeByte(lzwMinCodeSize);

      let offset = 0;

      while (raster.length - offset > 255) {
        out.writeByte(255);
        out.writeBytes(raster, offset, 255);
        offset += 255;
      }

      out.writeByte(raster.length - offset);
      out.writeBytes(raster, offset, raster.length - offset);
      out.writeByte(0x00);

      //---------------------------------
      // GIF Terminator
      out.writeString(';');
    };

    let getLZWRaster = (lzwMinCodeSize) => {
      let clearCode = 1 << lzwMinCodeSize;
      let endCode = (1 << lzwMinCodeSize) + 1;
      let bitLength = lzwMinCodeSize + 1;

      // Setup LZWTable
      let table = lzwTable();

      for (let i = 0; i < clearCode; i += 1) {
        table.add(String.fromCharCode(i));
      }
      table.add(String.fromCharCode(clearCode));
      table.add(String.fromCharCode(endCode));

      let byteOut = this.byteArrayOutputStream();
      let bitOut = bitOutputStream(byteOut);

      // clear code
      bitOut.write(clearCode, bitLength);

      let dataIndex = 0;

      let s = String.fromCharCode(_data[dataIndex]);
      dataIndex += 1;

      while (dataIndex < _data.length) {
        let c = String.fromCharCode(_data[dataIndex]);
        dataIndex += 1;

        if (table.contains(s + c)) {
          s = s + c;
        } else {
          bitOut.write(table.indexOf(s), bitLength);
          if (table.size() < 0xfff) {
            if (table.size() === (1 << bitLength)) {
              bitLength += 1;
            }
            table.add(s + c);
          }
          s = c;
        }
      }

      bitOut.write(table.indexOf(s), bitLength);

      // end code
      bitOut.write(endCode, bitLength);

      bitOut.flush();

      return byteOut.toByteArray();
    };

    return obj;
  }

  createImgTagPrima(width, height, getPixel, alt) {
    let gif = this.gifImage(width, height);
    for (let y = 0; y < height; y += 1) {
      for (let x = 0; x < width; x += 1) {
        gif.setPixel(x, y, getPixel(x, y));
      }
    }

    let b = this.byteArrayOutputStream();
    gif.write(b);

    let base64 = this.base64EncodeOutputStream();
    let bytes = b.toByteArray();
    for (let i = 0; i < bytes.length; i += 1) {
      base64.writeByte(bytes[i]);
    }
    base64.flush();

    let img = '';
    img += '<img';
    img += '\u0020src="';
    img += 'data:image/gif;base64,';
    img += base64;
    img += '"';
    img += '\u0020width="';
    img += width;
    img += '"';
    img += '\u0020height="';
    img += height;
    img += '"';
    if (alt) {
      img += '\u0020alt="';
      img += alt;
      img += '"';
    }
    img += '/>';

    return img;
  }

  createImgTag(cellSize, margin) {
    //TAMA�O DE IMAGEN
    cellSize = cellSize || 6;
    margin = (typeof margin === 'undefined') ? cellSize * 4 : margin;

    let size = this.getModuleCount() * cellSize + margin * 2;
    let min = margin;
    let max = size - margin;

    return this.createImgTagPrima(size, size, (x, y) => {
      if (min <= x && x < max && min <= y && y < max) {
        let c = Math.floor((x - min) / cellSize);
        let r = Math.floor((y - min) / cellSize);
        return this.isDark(r, c) ? 0 : 1;
      } else {
        return 1;
      }
    });
  }

  stringToBytes(s) {
    let bytes = new Array();
    for (let i = 0; i < s.length; i += 1) {
      let c = s.charCodeAt(i);
      bytes.push(c & 0xff);
    }
    return bytes;
  }

  base64DecodeInputStream(str) {
    let _str = str;
    let _pos = 0;
    let _buffer = 0;
    let _buflen = 0;

    let obj = {};

    obj.read = () => {

      while (_buflen < 8) {
        if (_pos >= _str.length) {
          if (_buflen == 0) {
            return -1;
          }
          throw new Error('unexpected end of file./' + _buflen);
        }

        let c = _str.charAt(_pos);
        _pos += 1;

        if (c === '=') {
          _buflen = 0;
          return -1;
        } else if (c.match(/^\s$/)) {
          // ignore if whitespace.
          continue;
        }

        _buffer = (_buffer << 6) | decode(c.charCodeAt(0));
        _buflen += 6;
      }

      let n = (_buffer >>> (_buflen - 8)) & 0xff;
      _buflen -= 8;
      return n;
    };

    let decode = (c) =>{
      if (c >= 0x41 && c <= 0x5a) {
        return c - 0x41;
      } else if (c >= 0x61 && c <= 0x7a) {
        return c - 0x61 + 26;
      } else if (c >= 0x30 && c <= 0x39) {
        return c - 0x30 + 52;
      } else if (c == 0x2b) {
        return 62;
      } else if (c == 0x2f) {
        return 63;
      } else {
        throw new Error('c:' + c);
      }
    };

    return obj;
  }

  createStringToBytes(unicodeData, numChars) {

    let unicodeMap = (function() {

      let bin = this.base64DecodeInputStream(unicodeData);
      let read = () => {
        let b = bin.read();
        if (b === -1) {
          throw new Error();
        }
        return b;
      };

      let count = 0;
      let unicodeMap2 = {};
      let condition = true;
      while (condition) {
        let b0 = bin.read();
        if (b0 === -1) {
          break;
        }
        let b1 = read();
        let b2 = read();
        let b3 = read();
        let k = String.fromCharCode((b0 << 8) | b1);
        let v = (b2 << 8) | b3;
        unicodeMap2[k] = v;
        count += 1;
      }
      if (count !== numChars) {
        throw new Error(count + ' != ' + numChars);
      }

      return unicodeMap2;
    }());

    let unknownChar = '?'.charCodeAt(0);

    return function(s) {
      let bytes = new Array();
      for (let i = 0; i < s.length; i += 1) {
        let c = s.charCodeAt(i);
        if (c < 128) {
          bytes.push(c);
        } else {
          let b = unicodeMap[s.charAt(i)];
          if (typeof b === 'number') {
            if ((b & 0xff) == b) {
              // 1byte
              bytes.push(b);
            } else {
              // 2bytes
              bytes.push(b >>> 8);
              bytes.push(b & 0xff);
            }
          } else {
            bytes.push(unknownChar);
          }
        }
      }
      return bytes;
    };
  }

  qr8BitByte(data) {
    let _mode = this.QRMode.MODE_8BIT_BYTE;
    let _data = data;
    let _bytes = this.stringToBytes(data);

    let obj = {};

    obj.getMode = () => {
      return _mode;
    };

    obj.getLength = function(buffer) {
      return _bytes.length;
    };

    obj.write = function(buffer) {
      for (let i = 0; i < _bytes.length; i += 1) {
        buffer.put(_bytes[i], 8);
      }
    };

    return obj;
  }
}

export {QRCodeGenerator};